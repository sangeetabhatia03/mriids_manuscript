...
equally important. For instance, \sangeeta{one of the ProMED outbreak analysts
...
Liberia and Sierra Leone, and are shown in Fig~\ref{fig:whopmhm}. \sangeeta{We
...
over. \sangeeta{Moreover, while ProMED and HealthMap did not record
...
The model performance varied depending on the \sangeeta{country and
...
\sangeeta{98.0\%} specificity at \sangeeta{50\% threshold}
(Fig~\ref{fig:alerts}A). \sangeeta{The
...
country (Fig~\ref{fig:rocnocases}). Out of the 8 missed alerts in this
case, 3 are in Liberia are towards the end of the epidemic, after
...
It is also worth noting that in each instance where our model failed to raise an
alert in a week, either a true or a false alert had
been raised in the recent weeks indicating a potential risk of spread
of the epidemic in that country.}
...
\sangeeta{These results} suggest that the model is able to
capture and even anticipate the spatial spread of the epidemic. 
\sangeeta{ Importantly, as the model is fitted to data from any of the
...
distance between two locations affects the flow \sangeeta{of
...
\sangeeta{of cases} between locations did not vary substantially over time, the probability
...
model) weighted by the infectivity at each country \sangeeta{(which
...
\sangeeta{the highest} relative
risk of acting as a source of importation \sangeeta{(relative risk $ >
...
with the data collated by WHO \sangeeta{and collected} through traditional surveillance methods.
...
\sangeeta{neither WHO nor} ProMED reported any case.
...
number was estimated on sliding windows of 28 days with a Gamma
prior with mean 1 and variance 0.25, using the R package
...
\begin{figure*}
\includegraphics{{../../ms-figures/2019-10-31_alerts_in_1_all_roc_ProMED_14_28}.pdf}
...
(light violet), 3 (dark pink) and 4 (green) weeks ahead. The
...
We used the \sangeeta{West African Ebola} incidence data 
...
\sangeeta{Since the data collected by ProMED and
  HealthMap are manually curated by outbreak analysts, we have used
  the word ``curate'' in refering to their data collection
...
situation reports from NGOs, and local and foreign medical teams.}
...
African continent into 5 groups and \sangeeta{forced each country in a
...
\acknow{The work in this manuscript was funded through the USAID
  ...
  not necessarily represent the official views of the USAID. SB, AC
  and PN acknowledge joint Centre funding from the UK Medical Research
  Council and Department for International Development (MR/R015600/1),
  funding from the UK National Institute for Health Research Health
  Protection Research Unit (NIHR HPRU) in Modelling Methodology at
  Imperial College London in partnership with Public Health England
  (PHE) (HPRU-2012-10080), and funding from the Bill and Melinda Gates
  Foundation (OPP1092240).}
