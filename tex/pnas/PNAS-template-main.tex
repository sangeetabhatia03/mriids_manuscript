...
on a map \cite{freifeld2008healthmap}. HealthMap
...
Intelligence from Open Sources (EIOS) surveillance system, developed by WHO. 
...
median $R_t$ estimates on sliding 4-week windows from ProMED or HealthMap
...
the main text (see SI Figs \ref{fig:pm26} and \ref{fig:pm28} for other results).
...
than the observed incidence (95\% CrI 0.0 - 32.0) based on the median relative mean absolute
...
Typically, individual forecasts were within 17.0\% (95\% CrI 0.0 - 0.8)
...
to ProMED and HealthMap data (SI Fig \ref{fig:perfds}), but increasing the calibration window
up to 6 weeks had little impact on the model performance (SI Fig \ref{fig:perftw}).
...
advance. For each week and each country in the region, our model
...
summarised in a ROC curve (Fig \ref{fig:alerts}A). Overall, our model achieved
high sensitivity but variable specificity. In general, using higher
percentiles as cut-off led to increased sensitivity but poorer
specificity. The maximum specificity was 89.5\% with a sensitivity of
84.7\%. Maximising the average between
sensitivity and specificity led to 88.6\% sensitivity and 88.8\%
specificity. The cut-off for raising an alert can thus be chosen based
on the relative costs of missed and false alerts.
...
99.6\% and the specificity is 15\%. 

Importantly, our model was able to accurately predict the presence of
cases in weeks following a week with
no observed cases. For instance, in May 2014, the model predicts the
...
(using 2.5th percentile) to 93.2\% (97.5th percentile). The high
sensitivity of the model suggests that the model is able to
...
pattern of infectious cases. Our method also provides estimates of
...
model predicted cases in countries, such as Côte d'Ivoire, where ProMED did not report any case.
...
surveillance system with an alert triggering further epidemiological investigations. 
...
\begin{figure*}
\includegraphics{{../../ms-figures/who-hm-promed-comparison}.pdf}
...
at least one data point was obtained using interpolation. The
...
to the length of one time window before the end.
In each case, the solid gray line is the median estimate and
...
done using data for all countries in Africa.}
\label{fig:pm24}
\end{figure*}
...
\caption{Model performance metrics stratified by
epidemic phase. The performance metrics 
...
transmissibility to be constant over the forecast horizon. If the 97.5th
...
where the 95\% Credible Interval of the R estimates contained 1.}
\label{fig:perfpm24}
\end{figure*}
...
Missed (red) alerts using the 97.5\textsuperscript{th} percentile of the
forecast interval as threshold. The figure only shows countries on the
African continent for which either the predicted incidence or the
...
week where there no observed cases in the previous week are shown
...
the actual source of importation. For each country, the figure shows only the risk
...
1. Model outputs for other data sources, model parameters and
...
to October 2016. The data set recorded the cumulative number of
...
\item
  ...
  cases).
\item
  Each unique record in ProMED/HealthMap data is associated with a
  unique alert id. Where multiple records were associated with the same
  alert id, we merged them by assigning the median of the case numbers
  to the record.
\item
...
estimated a single reproduction number for each location. To reduce
