...
params:
  ...
  twindow: 14
  N: 55
  incid: data/processed/promed_loglinear_wide_from_july.csv
---
...
If the convergence tests fail, stop right here.

```{r test}
flagged_fits <- here::here("data/stanfits",
                           "poorfits.csv")
rhats <- bayesplot::rhat(fit1)
ratios_cp <- bayesplot::neff_ratio(fit1)
rhats_check <- "pass"
neff_check <- "pass"
if (any(rhats > 1.1)) rhats_check <- "fail"
if (any(ratios_cp < 0.2)) neff_check <- "fail"
if (rhats_check == "fail" | neff_check == "fail") {
    data.frame(twindwow = params$twindow,
               tproj = params$tproj,
               rhats = rhats_check,
               neff = neff_check) %>%
        readr::write_csv(path = flagged_fits,
                         append = TRUE)
}    
