...
infiles <- purrr::map(
    ...
    ~ here::here(all_files[[.x]]$outdir,
         glue::glue("{Sys.Date()}_{.x}_trp_fpr_by_date_obs.csv")
  )
)
...
byparams <- split(overall, list(overall$time_window, overall$n.dates.sim), sep = "_")
params <- names(byparams)
...
purrr::walk(
    ...
    function(param) {
        ...
        p <- ggplot(df, aes(date_pred, tpr, col = datasource)) +
            geom_line()
        p <- p + geom_line(data = df,
                           aes(date_pred, fpr, col = datasource),
                           linetype = "dashed")
        p <- p + ylim(0, 1)
        p <- p + ylab("True/False Alert Rate") + xlab("")
        p <- p + geom_hline(yintercept = 0.5, alpha = 0.1)
        ...
        p <- p + incidplot + plot_layout(nrow = 2)
        outfile <- here::here(
            "ms-figures/si-figures/other",
            glue::glue("{Sys.Date()}_tpr_fpr_by_date_obs_ds_{param}.pdf")
            )
        ggplot2::ggsave(
            filename = outfile,
            plot = p,
