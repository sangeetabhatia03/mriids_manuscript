...
## Proportion of observations in 95% forecasts interval by country

```{r}
prop_in_ci_by_week_summary <-
    dplyr::group_by(
        prop_in_ci_by_week,
        country,
        week_of_projection
        ) %>% summarise(prop_in_ci = mean(prop_in_ci, na.rm = TRUE))

prop_in_ci_by_week_summary <- tidyr::spread(
    prop_in_ci_by_week_summary,
    key = week_of_projection,
    value = prop_in_ci
)
...
```{r}
knitr::kable(prop_in_ci_by_week_summary)
```
## Proportion of observations in 95% forecasts interval overall
...
## Proportion of observations in 95% forecasts interval by phase

```{r}
prop_in_ci_by_phase <- dplyr::group_by(
   prop_in_ci_by_week,
   country,
   ci
   ) %>% summarise(prop_in_ci = mean(prop_in_ci, na.rm = TRUE))

prop_in_ci_by_phase <- tidyr::spread(
   prop_in_ci_by_phase,
   key = ci,
   value = prop_in_ci
)
...
```{r}
knitr::kable(prop_in_ci_by_phase)
```
