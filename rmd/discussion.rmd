...
which relies on digital surveillance data from ProMED or /HealthMap to 
1) predict the short-term epidemic trajectory in currently affected 
countries, 2) quantifies the short-term risk of spread to other
countries and 3) for countries at risk, quantified where the risk
...
substantial quality issues remained in the quality and consistency of
the data feeds. Dealing with 
issues such as missing data and inconsistent records will be a key 
challenge in using these data for prospective real-time analysis. 

We argue that despite these challenges, digital surveillance tools can 
...
The model performs well at short projection horizon, i.e. up to two 
...
including Ebola epidemics, whereby a small number of cases generate a 
...
forecasting of the spatio-temporal spread of an outbreak. 
To further develop such approaches, it is important to establish an automated pipeline from data 
...
the usability of our model in near real time is to improve the
running time of the fitting and forward simulation. In the current 
implementation, the running time varies from up to 30 mins with projection being done 
...
of scale and unusual duration, there is a scope for optimising 
