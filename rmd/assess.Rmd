...
\[
 ...
 \frac{
 \sum_{s = 1}^N{\lvert I_{j, t} - \hat{I}^s_{j, t} \rvert}
 }
 { N *  (I_{j, t} + 1)}.
 \]
...
we used the relative median
absolute deviation about the median (MADM) to evaluate sharpness.
The sharpness \(s_{j, t}\) at time \(t\) at location \(j\)
is
\[
s_{j, t}(\hat{I}_{j, t}) = 
median(\left | \frac{\hat{I}^s_{j, t}  - median(\hat{I}_{j, t})}{median(\hat{I}_{j, t})} 
\right |).\]

We add 1 ti $\hat{I}_{j, t}$ to prevent division by 0.
