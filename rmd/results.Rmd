...
in this period. The data from ProMED is shown in Figure \@ref(fig:hmraw)

```{r hmraw, echo = FALSE, fig.cap = "Raw data from ProMED", fig.align = "center"}
knitr::include_graphics("ms-figures/promed_raw.png")
```
We derived the country specific daily and weekly incidence 
time series from these data after the following data cleaning 
and pre-processing:
...
filled in using linear or log-linear interpolation. 
 ...
 Further details of 
each step are presented in the supplementary text.
...
Result of each step the pre-processing of Promed and 
Healthmap data for Guinea in Figure \@ref(fig:hmclean).
...
Incidence time series from these various sourcesare shown in  
Figure \@ref(fig:incidcompare) for the three mainly affected 
...
number was estimated independently for each country in
using the R package EpiEstim [@cori2013new]. 
The estimated reproduction numbers are highly correlated.
