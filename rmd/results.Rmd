...
trends derived from ProMED, HealthMap and WHO data (left) and the
time-varying reproduction number (right) estimated using data from
the three sources. Solid coloured
lines in the right panel represent the median estimates for R and
the shaded areas represent the 95% CrI.")
...
over. The first situation report by the WHO reporting on 
cases between January and August 2014, was published only at the
...
```{r echo = FALSE, warning = FALSE}
g49cite <- fig_nums(name = "guinea49",
                    display = "cite")

```
`r g49cite` shows the
non-overlapping weekly forecasts for 4 weeks for Guinea, Liberia and 
Sierra Leone respectively. The length of the
time window used for estimating reproduction number is 4 weeks.
In each panel the top-most figure shows the projected weekly 
...
g49cap <- fig_nums(name = "guinea49",
                   caption = "Weekly observed and predicted
incidence in (A) Guinea, (B) Sierra Leone and (C) Liberia at different
points during the outbreak.The solid
dots represent the observed weekly incidence where the blue dots show
...
The middle figure in each panel shows the reproduction number used to
...
```{r echo = FALSE, fig.cap = g49cap, fig.align = "center"}
knitr::include_graphics("ms-figures/gin_sle_lbr_28.png")
```
