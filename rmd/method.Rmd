...
\end{bmatrix}
\]
where \(o_{k, j}\) is the observed incidence at time \(k\) at location
...
In practice, we have assumed a single \(p_{stay}\) for all three
countries in the analysis (Liberia, Guinea and Sierra Leone). For
estimating the time-varying reproduction number for each country, we
divided the time from the start of the outbreak upto to the time at
which we start projection into intervals and estimate a single
reproduction number for each time window. We varied the length of this
